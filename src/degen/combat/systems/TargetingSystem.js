...
export class TargetingSystem extends System {
  constructor(world) {
    ...
    // Track targets per entity (for server-side combat)
    this.entityTargets = new Map() // entityId → targetEntityId
  ...
  async init() {
    ...
    // Server mode - only track targeting relationships, no graphics
    if (this.world.network?.isServer) {
      console.log('[TargetingSystem] Server mode - tracking only')
      this.initialized = true
      return
    }

    // Client mode - full initialization with graphics
    console.log('[TargetingSystem] Client mode - full initialization')
  ...
  setTarget(entity) {
    ...
    // Store target for player entity
    const playerId = this.world.entities.player?.data?.id
    if (playerId && entity?.data?.id) {
      this.entityTargets.set(playerId, entity.data.id)
    }
  ...
  /**
   * Get target ID for specific entity
   * @param {string} entityId - Entity ID
   * @returns {string|null} Target entity ID
   */
  getTargetIdForEntity(entityId) {
    return this.entityTargets.get(entityId) || null
  }
